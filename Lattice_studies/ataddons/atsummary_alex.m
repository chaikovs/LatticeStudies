function varargout = atsummary_alex(varargin)
...
%
%  INPUTS:
%  1. lattice - AT lattice structure
%
...
%  NOTES: 
%  1. Still relies on GLOBVAL
%  2. THERING is used only if lattice is not provided as first
%  argument
% 
%
...
if ~isempty(varargin),
    if iscell(varargin{1}) % if lattice strucuture provided as first argument
        RING = varargin{1};
    else % else global
        global THERING;
        RING = THERING;
...
sum.circumference = findspos(RING, length(RING)+1);
...
[TD sum.tunes sum.chromaticity] = twissring(RING, 0, 1:length(RING)+1, 'chrom', 1e-8);
sum.compactionFactor = mcf(RING);
...
ii =0;
for i = 1:length(RING),
    if isfield(RING{i}, 'BendingAngle') && isfield(RING{i}, 'EntranceAngle')
        ii = ii +1;
        rho = RING{i}.Length/RING{i}.BendingAngle;
        [dI1,dI2,dI3,dI4,dI5,curHavg1(ii), Dxavg(ii)] = ...
            calcRadInt(rho,RING{i}.BendingAngle, ...
            alpha(i,1),beta(i,1),D_x(i),D_x_(i),...
            RING{i}.K,RING{i}.EntranceAngle,RING{i}.ExitAngle);
        ...
        %         sum.integrals(4) = sum.integrals(4) + 2*0.5*(D_x(i)+D_x(i+1))*RING{i}.Length/rho^3;
        ...
        sum.integrals(6) = sum.integrals(6) + RING{i}.PolynomB(2)^2*Dxavg(ii)^2*RING{i}.Length;
    end
end
...
if ~isempty(cavind)
    freq = RING{cavind(:,1)}.Frequency;
    v_cav = RING{cavind(:,1)}.Voltage;
else
    % Default
    freq = 352.202e6;
    v_cav = Vrf;
...
if DisplayFlag,
    ...
    fprintf(SeparatorString);
    fprintf('   Injection:\n');
    fprintf('   H: beta = % 8.3f [m] alpha = % 6.1e eta = % 6.3f [m] eta'' = % 6.1e \n', beta(1,1), alpha(1,1), D_x(1), D_x_(1));
    fprintf('   V: beta = % 8.3f [m] alpha = % 6.1e eta = % 6.3f [m] eta'' = % 6.1e \n', beta(1,2), alpha(1,2), D_y(1), D_y_(1));
