...
modelrmfile=fullfile(pwd,['RMmodel_test' speclab '.mat']);%
...
indqf1=find(atgetcells(ring,'FamName','QP31\w*'));
ring=atsetfieldvalues(ring,indqf1,'ForTuneF',1);                
indqd2=find(atgetcells(ring,'FamName','QP4\w*'));
ring=atsetfieldvalues(ring,indqf1,'ForTuneD',1); 
...
rerr=atsetfieldvalues(rerr,indQCor,'PolynomB',{1,2},pb2q.*(1+randn(size(indQCor))*0.5e-2)');
%% correction chain
...
neigenvectors=[...
    ...
    10,... % n eig orbit V
    0,... % skew quadrupole 
    20,... % normal quadrupole 
    8,... % fit normal quadrupole 
    0,... % fit dipole 
    0,... % fit skew quadrupole 
    ]; % number of eigenvectors 

diary('CorrChaintest.txt');
cororder=[1 2 ];%6 1 2 3 6 -1];
