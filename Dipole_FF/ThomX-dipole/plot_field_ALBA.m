...
% Bz=-Bz*0.7965; % *0.7965 pour ~50 MeV 200A
...
% hold off
% colorbar
% print('dipole_img_meas','-dpng','-r300')
...
hold off
colorbar
print('dipole_imgsc_meas','-dpng','-r300')
...
%
[ traj0 , ds ]      = make_traj_long(radius,0,teta);
[ trajp20 , dsp20 ] = make_traj_long((radius+0.020),+0.020,teta);
[ trajm20 , dsm20 ] = make_traj_long((radius-0.020),-0.020,teta);
[pole] = make_pole(radius,0.050,2*tetapole);
...
ylabel('x (mm)')
grid on
print('dipole_traj_meas','-dpng','-r300')
...
intbz0=trapz(cumsum(ds),-bz)*1000;%sum(-bz.*ds)*1000;
...
set(gcf,'color','w')
set(gca,'Fontsize',20)
plot((cumsum(ds) - ds(1))*1000   ,-bz,'-k');hold on
plot((cumsum(dsp20) - dsp20(1))*1000,-bzp20,'-b');
plot((cumsum(dsm20) - dsm20(1))*1000,-bzm20,'-r');hold off
legend('dx=0','dx=+20mm','dx=-20mm')
xlabel('s-traj (mm)')
ylabel('B (T)')
grid on
print('dipole_fieldtraj_meas','-dpng','-r300')

% s=cumsum(ds);
% save ThomX_dipole_trajGEOM160A.mat s bz

% integrated profil
xx=(-20:2:20);
intbz=[];
for dx=xx
    [ traj , ds ] = make_traj_long((radius+dx*1e-3),dx*1e-3,teta);
