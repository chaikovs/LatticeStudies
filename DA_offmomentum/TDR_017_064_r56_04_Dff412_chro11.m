...
% disp(['** Loading THOMX ring lattice ', mfilename]);
 ...
 L0 =  17.986715999999994; %17.986704000000003; % design circumference length [m]; 
...
SD0 = drift('SD0', 0.1000000E+00-dL - dL_track, 'DriftPass');

SD1S1B = drift('SD1S1B', 0.600000E-01 - dL_quad, 'DriftPass');
SD1S1 = drift('SD1S1', 0.190000E+00-LSX/2, 'DriftPass');
SD1S = drift('SD1S', 0.7500000E-01-LSX/2 - dL_quad, 'DriftPass');

SD3S2= drift('SD3S2', 0.1250000E+00-LSX/2 - dL - dL_track, 'DriftPass');

SD2 = drift('SD2', 0.2100000E+00 - 2*dL_quad, 'DriftPass');
SD31 = drift('SD31', 0.7537500E+00 - dL - dL_quad - dL_track, 'DriftPass');
...
SD5 = drift('SD5', 0.0600000E+00 - dL_quad, 'DriftPass');

SD3S = drift('SD3S', 0.7500000E-01-LSX/2 - dL_quad, 'DriftPass');
SD3S1 = drift('SD3S1', 0.1350000E+00-LSX/2 - dL_quad, 'DriftPass');

SD2SB = drift('SD2SB', 0.4000000E-01 - dL_quad, 'DriftPass');
SD2S = drift('SD2S', 0.1950000E+00 - dL - dL_track, 'DriftPass');

%% =======================
% dipole
%========================
L = 0.296351;%0.296233; %0.27646; % 0.296233 m @ 159.299 A; Opera gives 0.297124 @159.297 A
theta = 0.785398;
thetae = 0.03320;% 1.9 deg
thetas = 0.03320;% 1.9 deg
K =0.0;
beta_gap=0.042;
tracy_gap = beta_gap*2*0.41846;
fullgap = tracy_gap;
edge_effect1 = 1;
edge_effect2 = 1;
...
QP1 = quadrupole('QP1', LQP, -4.2578910000, QPassMethod);
QP2 = quadrupole('QP2', LQP, 8.658917000000001, QPassMethod);
QP3 = quadrupole('QP3', LQP,  -14.3351200000, QPassMethod);
QP4 = quadrupole('QP4', LQP, 16.47542000, QPassMethod);
QP31 = quadrupole('QP31', LQP, -9.8642310000, QPassMethod);
QP41 = quadrupole('QP41', LQP,  3.472987000, QPassMethod);
...
%  SX3 = sextupole('SX3', LSX,  -8.84477704272277/LSX *sx_on, SPassMethod);
 ...
 SX1 = sextupole('SX1', 0.1000000E-05, 1.95340860e6*sx_on,  SPassMethod); 
 SX2 = sextupole('SX2', 0.1000000E-05,  3.079704537294313e6 *sx_on, SPassMethod); 
 SX3 = sextupole('SX3', 0.1000000E-05,  -7.709460026520468e6 *sx_on, SPassMethod); 
...
%ELIST =[DEBUT SEPT str1 cell1 IP cell2 str2 str1 cell1 cell2 str2 FIN]';
 ELIST =[DEBUT SEPT str1 cell1 IP cell2 str2 RF str1 cell1 cell2 str2 FIN]';
